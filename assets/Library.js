...
function parseNums(num1pre, num2pre, mode) {
  if (checks) {
    if (1 > mode || mode > 5) throw new RangeError("dThe moe of the function must be from 1-5\n1: Addition\n2: Subtraction\n3: Multiplication\n4: Division\n5: Exponents");
    checkNumberString([num1pre, num2pre]);
  }
  var num = [null, num1pre, num2pre],
    skip = false,
    stringMode = [null, true, true],
    neg = [false, false, false],
    decimal = [0, 0, 0],
    numpos = [null],
    maxChar, numl;
  for (var i = 1; i < 3; i++) {
    if (num[i].num) neg[i] = num[i].isNeg, decimal[i] = num[i].decimals, num[i] = num[i].num, stringMode[i] = false;
    if (stringMode[i]) {
      var numisplit = num[i].split("-");
...
function multi() {
  ...
  var permfinal, a = [...arguments].sort((a,b)=>b.length-a.length);
  if (Array.isArray(a[0])) a = a[0];
...
function expo() {
  var tempexpo = function(num1, num2) {
    ...
    if (decimals[1] > 0) {
      ...
      if (num[2].num.length == 1 && num[2].num[0] == "1") return formatNums(num[1].num, decimals[0], false);
